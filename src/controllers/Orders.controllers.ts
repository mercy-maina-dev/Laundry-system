...
import * as getAllOrdersServices from '../Services/Orders.Services';// Import all functions from the UsersService module to handle business logic related to users
...
export const updateOrder=async(req:Request,res:Response)=>{
    ...
    try {
        ...
        const result=await getAllOrdersServices.updateOrder(id,orderData);
        if(!result){
            return res.status(404).json({error:'Order not found'});
        }
